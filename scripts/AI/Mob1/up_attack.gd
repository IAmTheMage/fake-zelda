...
func Enter():
	...
	timer.wait_time = 1.0
	if not timer.is_connected("timeout", Callable(self, "_on_up_attack_timer_timeout")):
		timer.connect("timeout", Callable(self, "_on_up_attack_timer_timeout"))
...
func Exit():
	timer.disconnect("timeout", Callable(self, "_on_up_attack_timer_timeout"))
